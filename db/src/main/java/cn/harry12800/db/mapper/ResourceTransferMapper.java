...
public interface ResourceTransferMapper { //extends CrudDao<ResourceTransfer> {
	...
	*/
	 ...
	 * @return
	 */
	ResourceTransfer findById(Long id);
	
	/**
	* 查询根据字段名称和数据
	**/
	List<ResourceTransfer> findByParam(@Param("propName") String propName,@Param("value") Object value);
	/**
	* 删除多行数据
	**/
	...
	* 查询多行数据
	**/
	List<ResourceTransfer> findByIds(Set<?> set);
	/**
	* 保存数据
	*/
