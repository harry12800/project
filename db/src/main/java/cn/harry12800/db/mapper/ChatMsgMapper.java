...
public interface ChatMsgMapper { //extends CrudDao<ChatMsg> {
	...
	*/
	 ...
	 * @return
	 */
	ChatMsg findById(Long id);
	
	/**
	* 查询根据字段名称和数据
	**/
	List<ChatMsg> findByParam(@Param("propName") String propName,@Param("value") Object value);
	/**
	* 删除多行数据
	**/
	...
	* 查询多行数据
	**/
	List<ChatMsg> findByIds(Set<?> set);
	/**
	* 保存数据
	*/
	int save(ChatMsg t);
